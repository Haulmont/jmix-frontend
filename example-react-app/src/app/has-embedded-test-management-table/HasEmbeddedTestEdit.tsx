...
import { HasEmbeddedTestManagement } from "./HasEmbeddedTestManagement";
...
import { HasEmbeddedTestEntity } from "../../jmix/entities/scr_HasEmbeddedTestEntity";
import { EmbeddableTestEntity } from "../../jmix/entities/scr_EmbeddableTestEntity";
...
@injectMainStore
@observer
class HasEmbeddedTestEditComponent extends React.Component<
  Props & WrappedComponentProps
> {
  dataInstance = instance<HasEmbeddedTestEntity>(HasEmbeddedTestEntity.NAME, {
    view: "hasEmbeddedTestEntity-fetch-plan",
  ...
  fields = ["stringAttr", "embeddableTestEntity.name"];
  ...
  isNewEntity = () => {
    return this.props.entityId === HasEmbeddedTestManagement.NEW_SUBPATH;
  };

  render() {
    if (this.updated) {
      return <Redirect to={HasEmbeddedTestManagement.PATH} />;
    }
    ...
    return (
      <Card className="narrow-layout">
        ...
        >
          <Field
            entityName={HasEmbeddedTestEntity.NAME}
          ...
          <Card
            ...
            title={
              <Msg
                entityName={HasEmbeddedTestEntity.NAME}
                propertyName="embeddableTestEntity"
              />
            }
          >
            <Field
              entityName={EmbeddableTestEntity.NAME}
              parentStringPath="embeddableTestEntity"
              propertyName="name"
              formItemProps={{
                style: { marginBottom: "12px" }
              }}
          ...
          <Form.Item style={{ textAlign: "center" }}>
            <Link to={HasEmbeddedTestManagement.PATH}>
  ...
  componentDidMount() {
    if (this.isNewEntity()) {
      this.dataInstance.setItem(new HasEmbeddedTestEntity());
    } else {
...
export default injectIntl(HasEmbeddedTestEditComponent);
